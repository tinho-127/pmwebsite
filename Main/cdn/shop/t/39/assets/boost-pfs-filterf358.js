...
Line 892:4 Transforming let to the configured target environment ("es5") is not supported yet
Line 894:6 Transforming let to the configured target environment ("es5") is not supported yet
Line 895:6 Transforming let to the configured target environment ("es5") is not supported yet
Line 896:6 Transforming let to the configured target environment ("es5") is not supported yet
Line 897:6 Transforming let to the configured target environment ("es5") is not supported yet
Line 898:6 Transforming let to the configured target environment ("es5") is not supported yet
Line 899:6 Transforming let to the configured target environment ("es5") is not supported yet
Line 902:8 Transforming let to the configured target environment ("es5") is not supported yet
Line 904:10 Transforming let to the configured target environment ("es5") is not supported yet
Line 909:10 Transforming let to the configured target environment ("es5") is not supported yet
...
Filter.prototype.afterRender = function(data) {
  ...
  // The product list data returned from API
    var index = 0;
  	var productDatas = this.data.products; 
    // console.log(productDatas)
  	// Your code here 
    let productItems = document.querySelectorAll(".boost-pfs-filter-product-item");
    productDatas.forEach(function(productData){
      let productDataId = productData.id
      let productDataVars = productData.variants;
      let oneTimeVariantPrice = 0;
      let subsVariantPrice = 0;
      let subsVariantPriceRaw1 = 0;
      let subsVariantPriceRaw2 = 0;

      productDataVars.forEach(function(productDataVar){
        let productDataVarTitle = productDataVar.title;
        if( productDataVarTitle.includes("Single")){
          let productDataVarPrice = productDataVar.price;
          oneTimeVariantPrice = productDataVarPrice * 12
          console.log({oneTimeVariantPrice})
        }
        if( productDataVarTitle.includes("12")){
          let productSubsDataVarPrice = productDataVar.price;
          subsVariantPrice = Number(productSubsDataVarPrice);
          subsVariantPriceRaw1 = Number(subsVariantPrice / 100 * 95) * 0.9;
          subsVariantPriceRaw2 = Number(subsVariantPriceRaw1 / oneTimeVariantPrice);
        }
      })
      let offer = Number(1 - subsVariantPriceRaw2) * 100;
      console.log({oneTimeVariantPrice, subsVariantPrice, subsVariantPriceRaw1, subsVariantPriceRaw2})
      productItems.forEach(function(productItem){
        productItemId = Number(productItem.getAttribute("data-id"));
        console.log({productItemId, productDataId})
        if(productItemId == productDataId){
          productItem.querySelector(".upto-off span").innerText = Math.round(offer)+"%";
        }
      })
      index++;
    })
